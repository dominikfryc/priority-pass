  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          'fixed top-1/2 left-1/2 z-50 flex flex-col w-[calc(100vw-2.5rem)] max-w-md max-h-[calc(100svh-2.5rem)] overflow-hidden -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-popover text-sm text-popover-foreground outline-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95',
          className,
        )}
        {...props}
      >
...
function DialogHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex flex-col gap-2 p-4 border-b shrink-0', className)}
      {...props}
    />
  )
}
